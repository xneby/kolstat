		...
		for of in oferty.oferty:
			...
			for train, start, end in plan:
				if not of.is_valid(train, start, end):
					valid = False
				tmp_result.extend( ('additional', of, type, train, znizka, value, get_distance(start, end)) for type, value, znizka in of.get_additional_prices(train, discounts))
			...
			if valid:
				tmp_result.extend(('base', of, type, (first_start, last_end), znizka, value, distance) for type, value, znizka in of.get_base_price(distance, discounts))
